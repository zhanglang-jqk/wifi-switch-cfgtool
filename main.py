...
SERIAL_PORT_TIMEOUT_MS = 0.5  # 串口超时时间,单位ms
...
class Paramer:
    ...
    def Connect(self):
        ...
        if (text == "打开"):
            try:
                self.ser = serial.Serial(self.port, 115200, timeout=SERIAL_PORT_TIMEOUT_MS)
    ...
    def ReadAll_cb(self):
        ...
        self.ser.write(msg)
        time.sleep(SERIAL_PORT_TIMEOUT_MS)  # wait for responsed to received buffer
